...
const SubscriptionCard = ({ isLoading, name, price, button, plan, isEssential, isStarter, createCheckoutSession, planId }: any) => {
    ...
    const saveDiscount = !isStarter ? (
        <div>
            ...
            <div className=' space-x-2'>
                <span className='px-2 py-[2px] text-white font-medium bg-red-500 text-xs rounded-lg'>Save $58</span>
                <span className='px-2 py-[2px] text-white font-medium bg-red-500 text-xs rounded-lg'>- $58%</span>
            </div>
        </div>
    ...
    return (
        <div className={`${cardClass} p-4 w-80 h-auto rounded-xl space-y-3`}>
            <p className={` text-lg font-semibold`}>
                {name} {isEssential && <span className='ml-3 px-2 py-[1px] text-[8pt] bg-white rounded text-purple-600'>Popular Plan</span>}
            </p>
            <p className={`${textClass} text-xs`}>In publishing and graphic design, Lorem placeholder text</p>
            <p><span className='text-5xl font-bold'>{price}</span>$/<span>{plan}</span></p>
            {saveDiscount}
            <hr />
            <div>
                <h1>Access to</h1>
                <ul className="list-none text-sm space-y-4 mt-2">
                    {isStarter && accessListStarter.map((item, index) => (
                        <AccessItem key={index} text={item.text} isEssential={isEssential} />
                    ))}
                    {isEssential && accessListEssential.map((item, index) => (
                        <AccessItem key={index} text={item.text} isEssential={isEssential} />
                    ))}
                    {!isStarter && !isEssential && accessListProfessional.map((item, index) => (
                        <AccessItem key={index} text={item.text} isEssential={isEssential} />
                    ))}
                </ul>
                <button
                    onClick={() => createCheckoutSession(planId)}
                    className={`${isEssential ? 'bg-white text-purple-600' : 'border-2 border-black'} mt-4 px-4 py-2 rounded-xl `}>
                    {button}
                </button>
            </div>
        </div>
