...
const SifelyLockAccessCodes = ({
  ...
  return (
    <>
      ...
      />
      <div className="px-4 w-[91%] mx-auto bg-gray-100 mb-2 py-3 rounded-md cursor-pointer select-none flex justify-between items-center transition duration-300 ease-in-out transform">
        <p className="text-gray-700 font-semibold text-sm">
          Access Codes{" "}
          <span className="text-slate-400">({passCodes?.length})</span>
        </p>
        <div className="flex items-center gap-4">
          <button
            onClick={handlePlusIconClick}
            className="outline outline-2 outline-indigo-600 mr-2 bg-white p-1 rounded-md"
          >
            <PlusIcon className="text-indigo-800  h-4 w-5" />
          </button>
        </div>
      </div>
      {showForm ? (
        <div className="px-8 mb-4">
          ...
          <div className="mb-2">
            ...
            <div className="flex items-center space-x-4">
              ...
              <input
                ...
                className="form-radio h-4 w-4 text-indigo-600 focus:ring-indigo-500"
              />
              <label htmlFor="startEnd" className="font-medium text-gray-700">
                Start/End Time
              </label>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-1">
            <button
              className={`rounded-md bg-indigo-600 w-16 px-2 py-1 text-base text-white shadow-sm hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500`}
              onClick={handleSave}
            >
              Save
            </button>
            <button
              className={`rounded-md bg-red-600 w-18 px-2 py-1 text-base text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500`}
        ...
        <div>
          <div className="px-6 mb-4">
            {passCodes?.length > 0 && (
              <>
                {passCodes
                  .slice(0, showAll ? passCodes.length : 2)
                  .map((code: any, index: any) => (
                    ...
                    >
                      <div className="p-1">
                        <h2 className="text-sm font-semibold text-gray-800 mb-1 capitalize">
                          {code?.keyboardPwdName}
                        </h2>
                        <div className="flex items-center justify-between">
                          <p className="text-xs text-gray-600">
                            Code: {code?.keyboardPwd}
                          </p>
                          <p className="text-xs text-gray-600">
                            Type:{" "}
                            {code?.keyboardPwdType == 2
                              ? "Permanent"
                              : "Period"}
                          </p>
                          <div className="relative">
                            <button
                              ...
                              onClick={() =>
                                handleDeletePassCode(code?.keyboardPwdId)
                              }
                            >
