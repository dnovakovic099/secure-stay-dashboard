  ...
  const handleSubmitForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (selectedGuideBook) {
        formData.append("id", selectedGuideBook.id.toString()); // Ensure id is a string
        response = await axiosInstance.put<any>(
          `${envConfig.backendUrl}/guides/updateGuides/${selectedGuideBook.id}`,
          formData
        );
      } else {
        response = await axiosInstance.post<any>(
          `${envConfig.backendUrl}/guides/addGuides`,
          formData
        );
      }
      console.log(response.status); // Use response.status instead of response.ok

      if (response.data) {
        const data: any = response.data; // Assuming data type is any for now
        if (selectedGuideBook) {
          ...
          setGuideBook((prevGuideBooks: any[]) => [...prevGuideBooks, data]);
        }
        handleClose();
      } else {
  ...
  return (
    <>
      <div className="flex justify-start items-center mt-4">
        <button
          className="text-base text-white bg-pink-500 hover:bg-pink-600 border border-pink-500 rounded-md py-2 px-4 focus:outline-none"
          onClick={handleOpen}
        >
          + Add Guidance
        </button>
      ...
      <div className="bg-white p-4 rounded-lg shadow-lg overflow-y-auto w-full mt-4">
